...
- **`public/img/aircraft/`** and **`img/aircraft/`** — the aircraft illustrations on the
  landing page (WebP, 1600 and 800 px), duplicated for the worker and the Pages origin.
- **`public/terms.html`** and **`public/privacy.html`** (copies at the repo root for Pages): the Terms of
  Service and Privacy Policy, served at `/terms` and `/privacy` and linked from every footer, the sign-up
  form and the account menu. Chartavia presents itself as an air charter broker under 14 CFR Part 295:
  the landing and legal-page footers, the email footer and the Terms disclosure box carry the required
  statements. New accounts record `users.terms_version` (`TERMS_VERSION` in `src/worker.js`) and
  `terms_at`; bump the version when the Terms change materially. The pages are generated once and then
  edited in place (keep `public/` and root copies identical).
- **`public/map.html`** — the airport map (Leaflet), generated by `pack.js` out
  of the bundle and served by the worker at `/map.html` for signed-in users.
  Tiles come from Mapbox (`dark-v11`) when a `MAPBOX_TOKEN` secret is set
  (`npx wrangler secret put MAPBOX_TOKEN`, a public `pk.` token, ideally
  restricted to the site URL), otherwise from OpenStreetMap under a CSS filter.
